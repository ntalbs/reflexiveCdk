...
function createAsg(stack: Stack): AutoScalingGroup {
  ...
  const alb = new ApplicationLoadBalancer(stack, 'Alb', {
    loadBalancerName: 'reflexive-rs-alb',
    vpc: vpc,
    internetFacing: true
  })
...
function createPipeline(stack: Stack, asg: AutoScalingGroup): Pipeline {
  let pipeline = new Pipeline(stack, 'ReflexiveRustPipeline', {
    pipelineName: 'ReflexiveRustPipeline',
  });
  ...
  let project = new Project(stack, 'Reflexive-Rust-Project', {
    environment: {
  ...
  let application = new ServerApplication(stack, 'ReflexiveRustApplication', {
    applicationName: 'ReflexiveRustApp',
  });
  let deploymentGroup = new ServerDeploymentGroup(stack, 'ReflexiveRustDeploymentGroup', {
    application,
    deploymentGroupName: 'ReflexiveRustDeploymentGroup',
